...
async def connect_to_voice(ctx):
    ...
    if not voice_channel:
        logger.warning(f"Користувач {ctx.author.name} спробував підключитись до бота без підключення до голосового каналу на сервері {ctx.guild.name}.")
    ...
    if ctx.voice_client is None:
        ...
        logger.info(f"Бот переміщено з голосового каналу '{old_channel.name}' до '{voice_channel.name}' на сервері '{ctx.guild.name}'.")
...
async def play_next(ctx):
    ...
    if not queue.empty():
        ...
        logger.info(f"Черга відтворення закінчилась на сервері {ctx.guild.name}. Бот відключається від голосового каналу.")
...
async def slash_play(interaction: discord.Interaction, url: str):
    ...
    logger.info(f"Користувач {interaction.user.name} запустив відтворення '{url}' через слеш-команду на сервері '{interaction.guild.name}'.")
...
async def stop(ctx):
    ...
    if voice_client and voice_client.is_playing():
        ...
        await voice_client.disconnect()
        logger.info(f"Відтворення зупинено та бот відключився від голосового каналу на сервері '{ctx.guild.name}' за запитом користувача {ctx.author.name}.")
        await clear_messages(ctx, 1)
    else:
...
async def set_volume(ctx, volume: int = 10):
    ...
    if voice_client:
        if 0 <= volume <= 100:
            ...
            logger.info(f"Гучність бота встановлено на {volume * 100}% користувачем {ctx.author.name} на сервері '{ctx.guild.name}'.")
        else:
...
async def clear_messages(ctx, amount: int = 0):
    try:
        ...
        if amount > 0:
            logger.info(f"Видалено {len(deleted)} повідомлень користувачем {ctx.author.name} в каналі`'{ctx.channel.name}' на сервері '{ctx.guild.name}'.")
    except discord.errors.NotFound:
        logger.warning(f"Спроба видалити повідомлення в неіснуючому каналі на сервері '{ctx.guild.name}'.")
    except discord.errors.Forbidden:
        logger.error(f"Бот не має дозволу на видалення повідомлень в каналі '{ctx.channel.name}' на сервері '{ctx.guild.name}'.")
